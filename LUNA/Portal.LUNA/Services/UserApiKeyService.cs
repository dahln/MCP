...
public class UserApiKeyService : IUserApiKeyService
{
    ...
    public UserApiKeyService(ApplicationDbContext db) => _db = db;

    public async Task<List<UserApiKeyDto>> GetByUserAsync(string userId) =>
        await _db.UserApiKeys.Include(k => k.McpServer).Include(k => k.Settings)
            .Where(k => k.UserId == userId).Select(k => MapToDto(k)).ToListAsync();

    public async Task<UserApiKeyDto?> GetByIdAsync(string id, string userId)
    {
        var entity = await _db.UserApiKeys.Include(k => k.McpServer).Include(k => k.Settings)
    ...
    public async Task<UserApiKeyDto> GenerateAsync(string userId, string mcpServerId)
    {
        ...
        var result = await _db.UserApiKeys.Include(k => k.McpServer).Include(k => k.Settings)
    ...
    public async Task<UserApiKeySettingDto> UpsertSettingAsync(string userApiKeyId, string userId, string key, string value)
    {
        ...
        var existing = await _db.UserApiKeySettings.FirstOrDefaultAsync(s => s.UserApiKeyId == userApiKeyId && s.Key == key);
        if (existing != null)
        {
    ...
    {
        var setting = await _db.UserApiKeySettings.Include(s => s.UserApiKey)
