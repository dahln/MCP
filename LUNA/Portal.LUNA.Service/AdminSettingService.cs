...
public interface IAdminSettingService
{
    ...
    Task<List<AdminSettingDto>> GetAllAsync();
}
...
public class AdminSettingService : IAdminSettingService
{
    ...
    public AdminSettingService(ApplicationDbContext db)
    {
        _db = db;
    }
    ...
    public async Task<List<AdminSettingDto>> GetAllAsync()
    {
        return await _db.AdminSettings
            .Select(s => new AdminSettingDto { Id = s.Id, Key = s.Key, Value = s.Value })
            .ToListAsync();
    }
}
